 ...
 */
function wrapError(err: unknown, filepath: string): Error & { code: string } {
...
export function createOPFSGitAdapter() {
  ...
  const promises = {
    ...
    async writeFile(
      ...
      _options?: { encoding?: Encoding; mode?: number }
    ): Promise<void> {
      try {
    ...
    async mkdir(filepath: string, _options?: { mode?: number }): Promise<void> {
      try {
    ...
    async readlink(_filepath: string): Promise<string> {
      ...
      const err = new Error('OPFS does not support symlinks') as Error & { code: string }
      err.code = 'ENOTSUP'
      throw err
    },
  ...
  function createStats(type: 'file' | 'dir', size: number, mtimeMs: number = Date.now()): Stats {
    return {
      type,
 ...
 */
let sharedAdapter: ReturnType<typeof createOPFSGitAdapter> | null = null

export function getOPFSGitAdapter(): ReturnType<typeof createOPFSGitAdapter> {
  if (!sharedAdapter) {
